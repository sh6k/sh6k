# Shakti Sagar Mohapatra

Computer Engineering student at OUTR, Bhubaneswar (4th Semester)

Interested in computer architecture, HPC, and systems programming.

## Projects
- **[cuda-matmul](https://github.com/sh6k/cuda-matmul)** — Parallel matrix multiplication in CUDA, 4.8x speedup over CPU on NVIDIA T4
- **[cpu-simulator](https://github.com/sh6k/cli-task-manager)** — CPU simulator in C with fetch-decode-execute cycle, pipelining, and hazard detection
- **[cli-task-manager](https://github.com/sh6k/Cpu-Simulator)** — CLI tool in C++ with linked list, merge sort, and file I/O

## Skills
C · C++ · Python · CUDA · Git

## Contact
mshakti166@gmail.com
